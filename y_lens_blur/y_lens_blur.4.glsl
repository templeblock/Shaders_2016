...
void main(void) {
	...
	vec4 input3 = texture2D(INPUT3, st);
	float blur_factor = texture2D(INPUT2, st).a;
	...
	for(float y = 0.0; y <= 1.0; y += 1.0 / s){
    	for(float x = 0.0; x <= 1.0; x += 1.0 / s){
			...
			if (! show_kernel) {
				...
				vec4 blur = texture2D(INPUT3, sample_coords);

				if (mipmap){
						float lod = min(width * 20 * blur_factor, max_lod);
						
						blur = texture2DLod(INPUT3, sample_coords, lod);
