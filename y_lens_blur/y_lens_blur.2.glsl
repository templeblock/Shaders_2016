...
void main(void) {
	...
	float depth = input1.a;
	depth = clamp(depth, 0.0001, .9999);
	//float depth = input1.a * far;
	//float fl = 1.0 - focal_length;
    //float c = aperture * (abs(depth - focal_distance) / depth) * (fl / (focal_distance - fl));
    //float p = c / .024;
    //float blur_factor = clamp(p, 0.0, max_coc);
	//float blur_factor = distance(depth, focal_distance) * aperture * ((max_focal - focal_length));

	float fp = focal_distance;

	if (pick_depth) {
		fp = texture2D(INPUT1, depth_pick).a;
	}

	//float blur_factor = distance(depth, fp) * aperture * ((max_focal - focal_length));

	//float blur_factor = smoothstep(fp - focal_length, depth - focal_length, depth);
	...
	gl_FragColor = vec4(color.rgb, blur_factor);
}
