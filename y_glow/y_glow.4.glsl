...
#define STRENGTH adsk_results_pass1
//#define VERTICAL 
...
vec4 gblur()
{
	...
	float br = blur_red * blur_amount * bias;
	float bg = blur_green * blur_amount * bias;
	float bb = blur_blue * blur_amount * bias;
	float bm = blur_matte * blur_amount * bias;
	...
	for(float i = 1; i <= support; i++) {
        a += gx * texture2D(adsk_results_pass3, (xy - i * dir) * texel);
        a += gx * texture2D(adsk_results_pass3, (xy + i * dir) * texel);
		energy += 2.0 * gx;
		gx *= gy;
		gy *= gz;
